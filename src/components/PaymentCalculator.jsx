...
export default async function PaymentCalculator() {
  ...
  const fields = [
    { id: "calc-price", label: t.price, value: price, unit: currency, step: 1000 },
    { id: "calc-down", label: t.downPayment, value: downPayment, unit: currency, step: 1000 },
    { id: "calc-months", label: t.term, value: months, unit: t.monthsUnit, step: 1 },
  ];

  const planRows = [
    [t.downPayment, `${Math.round((downPayment / price) * 100)}%`],
    [t.remaining, `${formatNumber(remaining)} ${currency}`],
    [t.term, termText],
  ];
  ...
  return (
    ...
    >
      ...
      <div className="mt-[clamp(28px,3.6vw,70px)] grid items-stretch gap-[clamp(20px,3.7vw,72px)] lg:grid-cols-[1075fr_693fr]">
        {/* Calculator form (not functional yet) */}
        <form className="rounded-[clamp(20px,2.25vw,44px)] bg-white pb-[clamp(22px,2.15vw,42px)] pl-[clamp(20px,2.35vw,46px)] pr-[clamp(20px,3.45vw,68px)] pt-[clamp(22px,2.3vw,46px)]">
        ...
        {/* Sample plan */}
        <aside className="flex flex-col rounded-[clamp(20px,2.25vw,44px)] bg-[#13271f] px-[clamp(20px,2.85vw,56px)] pb-[clamp(20px,1.85vw,36px)] pt-[clamp(24px,3vw,58px)] text-white">
          ...
          <div className="mt-auto pt-[clamp(20px,1.6vw,32px)]">
            <button
              type="button"
              className="h-[clamp(46px,3.8vw,74px)] w-full cursor-pointer rounded-full bg-[#ebe3c6] text-[clamp(12px,0.95vw,18px)] uppercase tracking-[0.02em] text-[#16201b]"
            >
              {t.talkToManager}
            </button>
          </div>
