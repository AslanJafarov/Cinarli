...
const imageFiles = { tower, balconies, interior, plan, greenery };
...
export default async function Gallery() {
  ...
  return (
    ...
    >
      ...
      <div className="mt-[clamp(28px,4.25vw,82px)] grid gap-[clamp(12px,1.85vw,36px)] lg:aspect-[1821/648] lg:grid-cols-[688fr_1096fr]">
        <figure className={`${tileClass} max-lg:aspect-[688/648]`}>
          ...
          >
            {gallery.photos.length} {ui.gallery.photos}
        ...
        <div className="grid gap-[clamp(12px,1.85vw,36px)] lg:grid-rows-[318fr_293fr]">
          {rows.map((row, rowIndex) => (
            <div
              key={rowIndex}
              className={`grid gap-[clamp(12px,1.85vw,36px)] max-sm:grid-cols-2 ${rowColumns[rowIndex]}`}
            >
              {row.map((image) => (
